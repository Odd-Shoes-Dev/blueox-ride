...
import type { ChurchCommissionSummary } from '@/shared/types'
import {
  ArrowLeft,
  Building2,
  Download,
...
type CommissionWithDetails = churchRepository.CommissionWithDetails
...
export default function AdminChurchPayoutsPage() {
  ...
  const fetchChurches = async () => {
    setLoading(true)
    try {
      const data = await churchRepository.getChurchCommissionSummaries()
      setChurches(data)
    } catch (error) {
  ...
  const fetchCommissions = async (churchId: string) => {
    ...
    try {
      const data = await churchRepository.getCommissionsForChurch(churchId)
      setCommissions(data)
    } catch (error) {
  ...
  const handleMarkAsPaid = async () => {
    ...
    try {
      ...
      for (const commissionId of selectedCommissions) {
        const { error } = await churchRepository.markCommissionPaid(
          commissionId,
          user?.id ?? '',
