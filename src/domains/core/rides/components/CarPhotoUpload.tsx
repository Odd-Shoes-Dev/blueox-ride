...
import { carPhotosRepository } from '@/shared/services/database'
...
export function CarPhotoUpload({
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const photoRecord = await carPhotosRepository.uploadCarPhoto(
        user.id,
        file,
        photos.length === 0 // First photo is primary
      )

      onPhotosChange([...photos, photoRecord])
  ...
  const handleRemove = async (photo: CarPhoto) => {
    try {
      await carPhotosRepository.removeCarPhoto(photo)
