...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const fetchProfile = async (userId: string, userEmail?: string, fullName?: string): Promise<User | null> => {
    return authRepository.getOrCreateUserProfile(userId, userEmail, fullName)
  }
  ...
  useEffect(() => {
    ...
    const initAuth = async () => {
      try {
        const { data: { session }, error } = await authRepository.getSession()
  ...
  const updateProfile = async (updates: Partial<User>): Promise<{ error: Error | null }> => {
    ...
    try {
      ...
      const { error } = await authRepository.updateUserProfile(user.id, updates)
